...
def simple_split(pretokenized_dir, tokenizer, max_len):
    ...
    simple_val = [{"facts": val_facts[i], "claims": val_claims[i], "outcomes": val_outcomes[i], "case_no": val_ids[i]} for i in range(len(val_facts))]
    simple_test = [{"facts": test_facts[i], "claims": test_claims[i], "outcomes": test_outcomes[i], "case_no": test_ids[i]} for i in range(len(test_facts))]
    simple_train = [{"facts": train_facts[i], "claims": train_claims[i], "outcomes": train_outcomes[i], "case_no": train_ids[i]} for i in range(len(train_facts))]
